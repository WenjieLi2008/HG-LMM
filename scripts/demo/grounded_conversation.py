...
if __name__ == '__main__':
    ...
    parser.add_argument('--image',
                        default='data/coco/val2017/000000033638.jpg', type=str)
    parser.add_argument('--text',
                        default='Briefly describe the picture',
                        type=str)
    parser.add_argument('--checkpoint',
                        default='checkpoints/frozen_deepseek_vl_1_3b_chat_unet_sam_l_refcoco_png.pth', type=str)
    parser.add_argument('--output_basedir', default='./')
    ...
    positive_ids = []
    str_places = [str_places[0], str_places[2]]
    ...
    for color_id, mask in enumerate(masks):
        image_np[mask] = image_np[mask] * 0.3 + np.array(colors[color_id]).reshape((1, 1, 3)) * 0.7

    image = Image.fromarray(image_np.astype(np.uint8))
    print(output_text, flush=True)
    print(phrases, flush=True)
    input_image_filename = os.path.basename(args.image)  # 如 '000000001296.jpg'
    image_basename = os.path.splitext(input_image_filename)[0]  # 如 '000000001296'
    output_folder = os.path.join(args.output_basedir, image_basename)
