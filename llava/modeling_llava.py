    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1:
                ...
                clip_hidden_states = [torch.zeros_like(image_outputs.hidden_states[i]) for i in
                                      range(len(image_outputs.hidden_states))]
                for i in range(len(image_outputs.hidden_states)):
                    clip_hidden_states[i] = image_outputs.hidden_states[i][:, 1:]
                    clip_hidden_states[i] = self.multi_modal_projector(clip_hidden_states[i])
        ...
        return LlavaCausalLMOutputWithPast(
            ...
            mask_ids=mask_ids,
            labels=labels,
        ),clip_hidden_states,inputs_embeds
