...
def evaluate_recall_with_mapping(coco_gt, coco_cap_gt, image_ids, pred_save_path, cap_pred_save_path, iou_threshold=0.5,
    ...
    for image_id in tqdm(image_ids):
        try:
            ...
            predicted_positives += len(dt_labels)
    ...
    precise = predicted_positives / actual_positives if predicted_positives > 0 else 0

    print(f"Recall: {recall:.3f}")
    print(f"precise: {precise:.3f}")


def get_predicted_image_ids(pred_save_path, cap_pred_save_path):
    pred_image_ids = set()

    with open(pred_save_path, 'r') as f:
        mask_preds = json.load(f)
        for pred in mask_preds:
            for key in pred:
                if isinstance(pred, dict):
                    if 'image_id' in pred:
                        pred_image_ids.add(pred['image_id'])
                    else:
                        for k, v in pred.items():
                            if isinstance(v, dict) and 'image_id' in v:
                                pred_image_ids.add(v['image_id'])

                if isinstance(pred, list):
                    for p in pred:
                        if isinstance(p, dict) and 'image_id' in p:
                            pred_image_ids.add(p['image_id'])

    with open(cap_pred_save_path, 'r') as f:
        cap_preds = json.load(f)
        for pred in cap_preds:
            if isinstance(pred, dict) and 'image_id' in pred:
                pred_image_ids.add(pred['image_id'])
...
def main():
    ...
    with open(f"{args.prediction_dir_path}/processed_image_ids.json", 'r') as f:
        all_images_ids = json.load(f)
