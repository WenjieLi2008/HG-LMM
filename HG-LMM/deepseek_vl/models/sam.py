...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        global_features = []
        hidden_states = []  # ← 新增：用于保存每一层的输出（即 hidden states）
        for i, blk in enumerate(self.blocks):
            x = blk(x)
            hidden_states.append(x)  # ← 新增：保存当前 block 的输出
            if self.sam_hd and blk.window_size == 0:
                global_features.append(x)
        ...
        hidden_states = F.interpolate(
            hidden_states.float(), size=(96, 96), mode="bilinear", align_corners=False
        ).to(x_dtype)
        hidden_states = self.downsamples(hidden_states)
