...
# 将 panopticapi 的路径添加到 sys.path
panopticapi_path = "/Storage/persenal/lwj/F-LMM/flmm/datasets/panopticapi"  # 替换为 panopticapi 文件夹的实际路径
sys.path.append(panopticapi_path)
    ...
    def __getitem__(self, index):
        ...
        for segment in data_sample['segments']:
            ...
            if len(segment['segment_ids']) == 0:
                # if ('and' in segment['utterance']) or (',' in segment['utterance']) or ('.' in segment['utterance']):
                #     mask_ids += [999] * len(segment_input_ids)
                # else:
                mask_ids += [-1] * len(segment_input_ids)
            else:
                mask_names.append(segment['utterance'])
