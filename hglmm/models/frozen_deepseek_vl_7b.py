...
class LCB(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, out_channels=128):
        ...
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 最终1/4下采样
...
class LDSI(nn.Module):
    ...
    def forward(self, dynamic_fused_image_embeds,mask_attentions,shallow_features):
        mask_attentions = self.conv960to384(mask_attentions)
...
class HGFrozenDeepseekVL(FrozenDeepseekVL):
    ...
    def _conversation(self, conversation, images):
        ...
        inputs_embeds, images_embeds, attns, clip_hidden_states = self.deepseek_vl.prepare_inputs_embeds(
            **prepare_inputs)
    ...
    def ground(self, positive_ids,  attention_maps, meta_data,
               clip_hidden_states, pixel_values, text_hidden_states, **kwargs):
        mask_attentions = []
        llm_text_embeds = []
...
if __name__ == '__main__':
    ...
    cfg = Config.fromfile('configs/deepseek_vl/hg_frozen_deepseek_vl_1_3b_chat_refcoco_png.py')
